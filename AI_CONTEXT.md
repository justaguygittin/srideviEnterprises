...
### Sprint 9 — UI Stabilization & Polish

A stabilization/polish pass across both portals, explicitly *not* a new-subsystem sprint — no schema change, no new route, no changed URL. Preceded by two scoped, read-only audits (general CSS/visual consistency, then a second pass targeted specifically at loading states, flash coverage, empty search results, pagination edge cases, missing feedback, mobile touch targets, and keyboard accessibility) so that only genuine, low-risk findings were implemented rather than cosmetic preference changes — most of the app (tables, modals, all five `confirm()` dialogs, theme color usage, pagination clamping on both customer and employee sides, and the searchable-combobox keyboard handling) came back from both audits already consistent and was deliberately left untouched. `templates/admin/*` and `routes/admin.py` were excluded from both audits and this sprint entirely — that blueprint is still an empty, unbuilt stub for the future administration sprint.

**Real bug fix — logout success message no longer persists on refresh/bookmark.** `routes/employee.py:logout()` previously redirected to `url_for("employee.login", logout_success=True)`, and `login()` read that query parameter to decide whether to show "You have been logged out successfully." Because it was a URL query string rather than a one-shot flash, bookmarking or refreshing `/employee/login?logout_success=True` re-showed the message indefinitely — the only message in the app that didn't behave like every other flash. Fixed by having `logout()` call `flash("You have been logged out successfully.", "success")` before its redirect, and removing the `logout_success`/`success_message` handling from `login()` and its duplicate custom `alert-success` block in `templates/employee/login.html` — the message now renders once, through the same global `components/flash_messages.html` include every other page already uses (`layout/base.html`), and is correctly gone on any subsequent request since Flask's flash queue is consumed on read. Login's own failure path (`error`, shown inline on the same POST response) was deliberately left untouched — it has no query-param/bookmark exposure, so there was no bug there to fix, and converting it too would have meant changing the route's POST→render flow to POST→redirect for no functional gain.

**Real bug fix — SQL `LIKE` search now escapes wildcard characters.** `_build_product_filters()` (`services/product_service.py`) and both `_build_enquiry_filters()`/`_build_customer_filters()` (`services/enquiry_service.py`) built their search pattern as `f"%{search}%"` with no escaping, so a literal `%` or `_` typed into any search box (Products, Enquiries, or Customers, customer-facing or employee-facing) was interpreted as a SQL wildcard and matched **every row** instead of doing a literal-text search — reproducible by pasting a stray `%` into any of these fields. Fixed with one small `_escape_like(value)` helper (backslash-escapes `\`, `%`, and `_` before the value is wrapped in `%...%`) duplicated identically in both service files, matching this codebase's established preference for a small amount of duplicated logic over a new shared module for something this size. Relies on MariaDB's default `sql_mode` (no `NO_BACKSLASH_ESCAPES`), which this project has never overridden. Verified live: searching `%` on `/products` now returns "Showing 0 products" (previously would have returned all 315+ rows); searching `_` correctly returns only the handful of products with a literal underscore in their name, brand, department, category, or model; a normal search term (`chair`) is unaffected (still 37 matches, highlighting and pagination intact).

**Loading-state gap closed on every write form that lacked one.** Before this sprint, only `product_form.html` and `inventory_transaction.html` disabled their Save/Confirm button and swapped in a `spinner-border` on submit; every other write path in the app had none. `templates/employee/department_form.html` (the only one of the three "reusable form" CRUD pages with zero `<script>` tags, despite doing a file upload), `templates/employee/login.html`, `templates/customer/enquiry_form.html`, and `templates/customer/contact.html` all gained the identical inline pattern (disable button, swap `innerHTML` to a spinner + "…ing" label) — the same convention this codebase already established, not a new one. `enquiry_form.html`'s script includes a null-check (`if (!form || !submitButton) return;`) since its form only exists in the page's `{% if product %}` branch. Verified end-to-end: a full valid Contact submission and a full valid product Enquiry submission both completed normally with no console errors; a Contact submission with an invalid email correctly showed the existing inline validation message and rendered a fresh, non-disabled button (a full-page re-render on a plain form POST naturally resets any client-side disabled state — no JS is needed to "undo" it).

**Login button now uses the standard `.btn.btn-primary` convention.** `templates/employee/login.html`'s Login button was the only primary form-submit button in the entire app using a custom gradient class (`.login-btn`) instead of Bootstrap's `.btn-primary`, which every other Save/Submit button already uses. The custom class and its hover-lift transform were removed; a small `.login-card button[type="submit"] { margin-top: 1rem }` rule preserves the original spacing.

**Empty-state consistency fixes on the two pages that had drifted from the rest of the app.** `templates/employee/products.html`'s "No matching products found" empty state was the one outlier among five sibling pages (Dashboard, Enquiries, Customers, Inventory's four lists) that all share one pattern (bare `<i>` icon, no wrapper div, optional `.empty-state-subtext` line) — it wrapped its icon in an extra `<div class="empty-state-icon">` and had no subtext. Now matches its siblings exactly, plus a subtext line ("Try adjusting your search or filters."). `templates/customer/categories.html` had the least-polished empty state on the whole customer site — both its "no departments configured yet" and "no search matches" states were bare gray `<p>` text with no icon, in contrast to the Products page's much richer empty state. Both now render an icon (`fa-shapes` / `fa-magnifying-glass`, reusing icons already used elsewhere on this same page) plus the existing message, styled via two new rules in `categories.css`. The client-side live-search toggle (`initCategorySearch()` in `main.js`) needed no change — it only ever toggles the `d-none` class on the whole element, never touches its inner markup.

**Customer-site mobile touch targets brought to the same ~44px minimum Sprint 7 already established for the Employee Portal.** That earlier sprint's audit was explicitly Employee-Portal-only; the customer site never got an equivalent pass. Three elements were found and fixed, each scoped to its existing mobile media query so desktop sizing is untouched: the homepage Featured Categories Slider's `.slider-arrow` (`categories.css`, used by `templates/components/categories.html`) shrank from its normal 44px down to 36px under `max-width: 768px` — now stays at 44px on mobile too. The Products search field's clear-× button (`.search-bar-clear`, `static/css/components/search.css`, shared globally with the navbar search) was a fixed 28px with no mobile override anywhere — now gets a new `@media (max-width: 575.98px)` block bumping it to 44px, matching the breakpoint `products.css`'s own pagination touch-target fix already uses; desktop stays 28px, confirmed via computed-style check at both viewport sizes. The mobile filter-drawer's close button (`templates/customer/products.html`, Bootstrap's unstyled default `.btn-close`, ~24px) — the primary way to dismiss the drawer on a phone — now has a scoped `.filter-drawer .offcanvas-header .btn-close { width: 44px; height: 44px; padding: 0 }` rule in `products.css`; scoped to `.filter-drawer` specifically (rather than a global `.btn-close` override) so it can't affect the flash-message dismiss button or any Bootstrap modal's close button elsewhere in the app. All three verified via computed-style checks at a 375px viewport (44×44px) and confirmed unchanged at desktop width.

**Deliberately not changed** (found during audit, judged cosmetic/low-value or already a deliberate convention, per the sprint's explicit "no architectural changes, no cosmetic preference changes" instruction): hand-rolled notice boxes (`.invoices-unavailable`, `.deactivation-note`) left as-is rather than folded into Bootstrap `alert` classes; `.action-link` vs. `.btn-sm` row-action style difference between Products/Departments and Enquiries/Customers; `.category-grid-card`'s 18px border-radius vs. the customer site's 14px norm elsewhere; `employee_nav.html`'s older `current_page`-variable active-state mechanism vs. `navbar.html`'s `request.endpoint`-based one; `main.js` loading harmlessly on employee pages that never use it; `static/css/base.css` (0 bytes, loaded on every page) — the user confirmed this is intentionally reserved for future application-wide styles and should not be repurposed or have its `<link>` removed.
